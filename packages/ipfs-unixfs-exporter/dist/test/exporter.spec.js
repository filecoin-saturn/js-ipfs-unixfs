...
describe('exporter', () => {
    ...
    async function addTestFile(options) {
    ...
    async function addAndReadTestFile(options) {
    ...
    async function checkBytesThatSpanBlocks(layout) {
    ...
    it('exports a directory', async () => {
        ...
        expect(files.map((file) => file.path)).to.be.eql([
        ...
        expect(files
            .map(file => file.type === 'file' && file.unixfs.type === 'file')).to.deep.equal([
            true,
            false,
            false
        ]);
    ...
    it('exports file with data on some internal and leaf nodes', async () => {
        ...
        const data = uint8ArrayConcat(await all(file.content()));
        expect(data).to.deep.equal(Uint8Array.from([
    ...
    it('should support being used with readable-stream', async () => {
        ...
        while (dataSizeInBytes <= 10000000) {
            ...
            const readableStreamToBytes = async (readableStream) => {
                return new Promise((resolve, reject) => {
                    const chunks = [];
...
//# sourceMappingURL=exporter.spec.js.map