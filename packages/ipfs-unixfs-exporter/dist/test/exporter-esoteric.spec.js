...
describe('exporter esoteric DAGs', () => {
    ...
    async function storeBlock(buf, codec) {
    ...
    it('exports an unbalanced DAG', async () => {
        ...
        expect(data).to.deep.equal(concat(leaves.map(l => l.buf)));
    });
    ...
    it('errors on DAG with blocksizes that are too large', async () => {
        ...
        const unixfs = new UnixFS({
            ...
            blockSizes: [
                BigInt(leaves[0].buf.byteLength),
                BigInt(leaves[1].buf.byteLength + 5),
                BigInt(leaves[2].buf.byteLength)
            ]
        });
        const rootNode = {
    ...
    it('errors on DAG with blocksizes that are too small', async () => {
        ...
        const unixfs = new UnixFS({
            ...
            blockSizes: [
                BigInt(leaves[0].buf.byteLength),
                BigInt(leaves[1].buf.byteLength - 2),
                BigInt(leaves[2].buf.byteLength)
            ]
        });
        const rootNode = {
...
//# sourceMappingURL=exporter-esoteric.spec.js.map